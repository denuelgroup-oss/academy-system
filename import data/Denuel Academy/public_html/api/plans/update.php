...
function ensure_plan_column_update($conn, $columnName, $alterSql)
{
...
ensure_plan_column_update($conn, 'duration_unit', 'ALTER TABLE plans ADD COLUMN duration_unit VARCHAR(20) NOT NULL DEFAULT "month"');
ensure_plan_column_update($conn, 'auto_renew_clients', 'ALTER TABLE plans ADD COLUMN auto_renew_clients TINYINT(1) NOT NULL DEFAULT 0');
ensure_plan_column_update($conn, 'control_available_start_dates', 'ALTER TABLE plans ADD COLUMN control_available_start_dates TINYINT(1) NOT NULL DEFAULT 0');
ensure_plan_column_update($conn, 'currency', 'ALTER TABLE plans ADD COLUMN currency VARCHAR(3) NOT NULL DEFAULT "CDF"');
...
if ($id <= 0 || $title === '' || !in_array($type, $allowed, true) || $amount <= 0) {
    json_response(['message' => 'id, title, valid type and positive amount are required'], 400);
}

$stmt = $conn->prepare(
    'UPDATE plans
     SET title = ?, plan_type = ?, duration_unit = ?, classes_count = ?, clients_count = ?,
         make_up = ?, amount = ?, currency = ?, billing_cycle = ?, auto_renew_clients = ?, control_available_start_dates = ?
     WHERE id = ?'
);
$stmt->bind_param(
    'sssiiidssiii',
...
if (!$stmt->execute()) {
    json_response(['message' => 'Failed to update plan'], 500);
}
