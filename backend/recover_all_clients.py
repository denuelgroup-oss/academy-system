...
# Connect to the most complete SQLite backup
...
for row in sqlite_clients:
    row_dict = dict(row)
    first_name = row_dict['first_name'] or ''
    last_name = row_dict['last_name'] or ''
    
    # Check if client already exists in MySQL by name
    ...
    if existing:
        print(f"  ⊘ {first_name} {last_name} - already exists (ID: {existing.id})")
        skipped += 1
        continue
...
print(f"\n=== Sync Complete ===")
print(f"Imported: {imported} new clients")
